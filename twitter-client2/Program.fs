        ...
        | "Follow" ->
            if loggedIn then
                ...
                if not <| (id = followid) then 
                    let followerUserId = clientSystemName + (string)followid
            ...
            if loggedIn then
                ...
                if index < embedWordList.Length/2 then 
                    ...
                    if not <| (id = mentionId) then 
                        mention <- "@" + clientSystemName + (string)mentionId
        ...
        | "QueryByHashTagOrMention" ->
            if loggedIn then
                ...
                if index < embedWordList.Length/2 then 
                    ...
                    let mentionId = Random().Next(1, id)
                    if not <| (id = mentionId) then 
                        embedWord <- "@" + clientSystemName + (string)mentionId
...
let ClientConn numOfUsers (clientSystem:ActorSystem)  (mailbox: Actor<_>) =
    ...
    let rec loop() = actor{
        ...
        match message with
        | "SpawnUsers" ->
            userId <- clientSystemName + (string)userNumber
